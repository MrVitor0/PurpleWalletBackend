...
exports.deleteInvestment = async (req, res) => {
    try {
        const { id_investment_history } = req.params;
        const id_user = req.user.id;
     
       //find one InvestmentModel that has id_user = id_user and has one InvestmentHistoryModel that has id = id_investment_history
        ...
        res.status(200).send({ msg: investment });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
    }
